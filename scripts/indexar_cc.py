"""
indexar_cc.py — Carga el Código Civil boliviano en leyes_chunks (pgvector).

Uso (dentro del contenedor ai):
    python scripts/indexar_cc.py /ruta/al/codigo_civil.pdf

O desde fuera:
    docker compose exec ai python scripts/indexar_cc.py /storage/documentos/codigo_civil.pdf
"""
...
PATRONES = {
    ...
    # Múltiples formatos por errores OCR en el PDF del CC:
    #   Art. 1o.  /  Art. 436.-  /  Art. 243-  /  Art. 66-..  /  ARTÍCULO 1.
    "articulo": re.compile(
        r"^ART[ÍI]CULO\s+\d+\."
        r"|^Art\.\s+\d+[o0]?[\.\-]{0,3}",
        re.IGNORECASE,
...
def main():
    if len(sys.argv) < 2:
        print("Uso: python scripts/indexar_cc.py <ruta_pdf>")
        sys.exit(1)
