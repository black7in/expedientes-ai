"""
indexar_cpc.py — Carga el Código Procesal Civil boliviano en leyes_chunks (pgvector).

Uso (dentro del contenedor ai):
    python scripts/indexar_cpc.py /ruta/al/codigo_procesal_civil.pdf

O desde fuera:
    docker compose exec ai python scripts/indexar_cpc.py /storage/documentos/cpc.pdf
"""
...
NOMBRE_LEY   = "Código Procesal Civil"
...
PATRONES = {
    ...
    # Solo "ARTÍCULO 15." — el CPC tiene formato limpio y consistente.
    # Regex estricto para no confundir referencias como "Art. 15 CC"
    # que aparecen en el cuerpo de los artículos.
...
def main():
    if len(sys.argv) < 2:
        print("Uso: python scripts/indexar_cpc.py <ruta_pdf>")
        sys.exit(1)
